...
- 📱 多账号支持
- 🔐 扫码登录（推荐）
- 🍪 粘贴 Cookie 添加（推荐用于扫码失败 / 海外无法接收短信验证码场景）
- 🔑 账号详情中可补充用户名 / 登录密码用于自动续期登录
- ⚡ 账号启用/禁用
- 🤖 AI 议价配置
...
⚠️ **安全提示：登录系统后会强制弹出修改默认密码引导，请立即修改！** 系统也提供了首次使用引导，会带你完成基本配置。
...
登录管理后台后会自动弹出引导窗口，依次完成以下步骤即可：

1. **修改默认管理员密码**（强制）- 系统检测到默认密码会强制弹出修改窗口
2. **添加闲鱼账号** - 在「账号管理」页有两种方式：
   - 扫码登录（推荐境内、能正常收短信用户）
   - 粘贴 Cookie（推荐扫码失败、海外接收不到短信验证码用户；详细步骤见弹窗内说明）
3. **配置关键词回复 / 卡密 / 发货规则** - 在「关键词」「卡密」「规则」三个页面分别完成配置
4. **（可选）启用 AI 议价 / 邮件通知 / SMTP** - 在「系统设置」中按需开启

> ℹ️ **关于 Xianyu 网页版会话每隔几小时强制下线（issue #36）**：这是闲鱼平台自身的安全策略，本项目无法绕过。
> 推荐做法：在账号管理中预先填写「用户名 / 密码」，让系统在 Cookie 失效后通过浏览器自动续期；或使用「粘贴 Cookie」方式快速恢复登录。
